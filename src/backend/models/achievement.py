...
from sqlalchemy import Boolean, DateTime, ForeignKey, Integer, String, Text, func, UniqueConstraint, Float
...
class UserAchievement(Base):
    ...
    __table_args__ = (
        UniqueConstraint('user_id', 'achievement_id', 'period_key', name='uq_user_achievement_period'),
    )
...
class CommunityAchievement(Base):
    ...
    # Time constraints
    time_window_hours: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)  # Time window for goal (e.g., 24 for daily)
