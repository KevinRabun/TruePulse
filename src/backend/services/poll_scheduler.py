...
class PollScheduler:
    ...
    async def get_current_poll(self) -> Optional[Poll]:
        ...
        special_poll_query = select(Poll).where(
            and_(
    ...
    async def get_previous_poll(self) -> Optional[Poll]:
        ...
        if not poll:
            fallback_query = select(Poll).where(
                Poll.status == PollStatus.CLOSED.value,
            ).order_by(Poll.closed_at.desc()).limit(1)
    ...
    async def get_upcoming_polls(self, limit: int = 5) -> list[Poll]:
        ...
        ).order_by(Poll.scheduled_start.asc()).limit(limit)
    ...
    async def _generate_poll_from_events(self) -> Optional[Poll]:
        ...
        try:
            ...
            events = await aggregator.fetch_trending_events(
                categories=["politics", "business", "technology", "health", "environment"],
                limit=20,
            )
...
async def schedule_flash_poll(
    ...
    logger.info(
        f"Scheduled Flash Poll: '{question[:50]}...' for {start_utc} (1 hour)"
    )
